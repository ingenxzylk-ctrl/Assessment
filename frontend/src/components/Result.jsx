...
export default function Result() {
  ...
  const displayUserPhoto =
    (typeof realFrontImage === "string" ? realFrontImage : realFrontImage?.dataUrl) ||
  ...
  const hasDandruff =
    stateDumpString.includes("dandruff") && !stateDumpString.includes("no-dandruff");

  const recommendedBundle = !requiresDoctorConsultation
    ? getRecommendedBundle(gender, aiPredictedStageNumber, hasDandruff)
    : null;
  ...
  const stageDescription = requiresDoctorConsultation
    ...
    : rawAnalysis.stageDescription ||
      `Our multi-modal assessment indicates localized phase shifts matching a ${getStageTitle()} signature layout.`;
  ...
  const regrowthOutlook = requiresDoctorConsultation
    ...
    : rawAnalysis.regrowthOutlook ||
      `Your target configuration responds exceptionally well to your prescribed comprehensive treatment package layout.`;

  const confidencePercent = rawAnalysis.aiConfidence
    ? Math.round(rawAnalysis.aiConfidence * 100)
    : 94;

  const aiReasoning =
    rawAnalysis.aiReasoning ||
    `Visual tracking mapping isolated root thinning areas consistent with a distinct signature layout configuration.`;
  ...
  const contributingFactors = requiresDoctorConsultation
    ? [
        {
          tag: "genetic",
          label: "Advanced Genetic Miniaturization",
          description:
            "Long-standing enzyme paths causing closure of active root fields.",
        },
      ]
    : [
        {
          tag: "genetic",
          label: isFemale ? "Hormonal Hair Vector" : "Genetic Predisposition Vector",
          description: isFemale
            ? "Receptor pathways causing micro-thinning along center hair parts over time."
            : "Hormonal shifts triggering cellular variations along target active follicles.",
        },
        {
          tag: "stress-related",
          label: "Stress Adaptation Metrics",
          description:
            "Fluctuating cortisol levels forcing root clusters prematurely into resting telogen phases.",
        },
        {
          tag: "scalp-related",
          label: "Scalp Shield Environment",
          description: hasDandruff
            ? "Surface microbial activity disrupting lipid barrier consistency."
            : "Standard external cellular balance status.",
        },
      ];
  ...
  subtitle: `Bundle ${recommendedBundle.bundleNumber} • 1 Month Kit`,
  bundleNumber: recommendedBundle.bundleNumber,
  stage: aiPredictedStageNumber,
  gender,
});
    alert(`${recommendedBundle.bundleTitle} has been added to the cart!`);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-8 items-start w-full !text-left">
        <div className="lg:col-span-6 bg-white rounded-[32px] p-6 md:p-8 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100 space-y-8 w-full !text-left block">
          <div className="w-full !text-left block">
            <span className="text-xs font-bold tracking-[0.1em] text-gray-400 uppercase block !text-left">
              Assessment Report
            </span>
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-between items-start gap-6 mt-4 w-full !text-left">
              <div className="space-y-3 flex-1 !text-left block">
                ...
                <p className="text-lg text-gray-700 font-medium !text-left">
                  ...
                  <span className="text-[#064e3b] font-bold">{getStageTitle()}</span>
                </p>
                <div className="pt-2 !text-left block">
                  <span className="text-xs font-bold text-gray-400 block uppercase tracking-wide !text-left">
                    Start Seeing Results In
                  </span>
                  <span className="text-2xl font-black text-gray-900 !text-left block">
                    {timelineMap[aiPredictedStageNumber] || "6 Months"}
                  </span>
                </div>
              </div>
          ...
          {analysisMissing && (
            <div className="p-4 bg-red-50 border border-red-200 rounded-2xl text-sm text-red-800 font-medium leading-relaxed !text-left w-full block">
              ...
              <p className="text-xs text-red-600 mt-1.5 !text-left">
                Your questionnaire answers were saved, but the photo analysis didn't finish.
                Please retake the scalp scan to get your actual stage.
              </p>
              <button
                type="button"
                onClick={() => {
          ...
          <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-200 text-[11px] text-yellow-800 font-mono !text-left w-full block">
            🔧 DEBUG — gender: <b>{gender}</b> | self-reported: <b>{reportedStage}</b> | AI
            predicted stage: <b>{String(aiPredictedStageNumber)}</b> | bundle:{" "}
            <b>{recommendedBundle?.bundleNumber ?? "none"}</b> | cart name:{" "}
            <b>{recommendedBundle?.bundleTitle ?? "none"}</b>
          </div>
          ...
          <div className="space-y-4 w-full !text-left block">
            <h4 className="text-base font-bold text-gray-900 tracking-tight !text-left">
              Your Hair Fall Root Causes
            </h4>
              ...
              {contributingFactors.map((factor) => (
                <button
                  ...
                  className={`p-3 rounded-2xl border flex flex-col items-center gap-1.5 text-center transition-all cursor-pointer ${
                    activeTab === factor.tag
                      ? "border-[#064e3b] bg-[#064e3b]/5 text-[#064e3b] font-bold"
                ...
                >
                  ...
                  <span className="text-xs tracking-tight truncate max-w-full">
            ...
            {contributingFactors.find((f) => f.tag === activeTab) && (
              <div className="p-4 bg-gray-50 rounded-2xl text-xs text-gray-600 border border-gray-100 !text-left space-y-1 w-full block">
                <p className="font-bold text-gray-900 !text-left">
                  {contributingFactors.find((f) => f.tag === activeTab).label}
                </p>
                <p className="leading-relaxed !text-left">
                  {contributingFactors.find((f) => f.tag === activeTab).description}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="p-5 rounded-2xl bg-gray-50 border border-gray-200/60 w-full !text-left block">
            <div className="flex justify-between text-xs mb-2 font-bold w-full !text-left">
              <span className="text-gray-500 uppercase tracking-wider !text-left">
                AI Confidence Matrix Ratio
              </span>
            ...
            <div className="w-full block">
              {state ? (
                ...
                <div className="text-center text-xs text-gray-400 py-2">
                  Awaiting image extraction matrix...
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-4 bg-white rounded-[32px] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100 space-y-6 w-full !text-left block">
          <div className="!text-left w-full block">
            <h3 className="text-lg font-bold text-gray-900 tracking-tight !text-left">
              Start Your Journey With Just 1 Month Kit
            </h3>
            <p className="text-xs text-gray-400 mt-1 !text-left">
              Clinically targeted formula system mapped configurations.
            </p>
          </div>
          ...
          {requiresDoctorConsultation ? (
            <div className="p-5 rounded-2xl bg-amber-50 border border-amber-200 !text-left space-y-2 w-full block">
              <h4 className="font-bold text-sm text-amber-900 flex items-center gap-1 !text-left">
                ⚠️ Direct Orders Restricted
              </h4>
              <p className="text-xs text-amber-700 leading-relaxed !text-left">
                Due to advanced hair loss depletion parameters, this configuration layout
                requires custom medical prescription setups. Please arrange an entry
                consultation exam with an authorized trichologist practitioner using the
                link below.
              </p>
            </div>
          ) : (
            <div className="space-y-4 w-full !text-left flex flex-col justify-start items-start">
              {recommendedBundle && (
                <div className="px-1 w-full">
                  ...
                  <p className="text-[11px] text-gray-400 mt-0.5">
                    Bundle {recommendedBundle.bundleNumber} • 1 Month Treatment Kit
                  </p>
                </div>
              )}
              ...
              {(recommendedBundle?.items ?? [])
                .map((prod) => {
                  const formatted = formatProductName(prod.name);
                  return formatted ? { ...formatted, shortName: prod.name } : null;
                })
