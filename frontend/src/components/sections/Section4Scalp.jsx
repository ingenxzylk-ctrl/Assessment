...
export default function Section4ScalpAssessment({ onComplete, onBack }) {
  ...
  const handleTriggerAnalysis = async () => {
    if (isFemale && (!images.front || !images.side || !images.back)) {
      setError("Please provide Front, Side, and Back images to proceed.");
      return;
    }
        ...
        <div className="bg-white rounded-[32px] p-8 md:p-10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
          <div className="mb-6 text-center">
            ...
            <p className="text-gray-500 mt-2 text-base">
              You will need to upload or take a snapshot of {isFemale ? "three specific views" : "two distinct views"}.
            </p>
          </div>

          {/* 🟢 FIXED: Mobile layout shifts to a balanced 2-column stack, aligning 3rd card on its own row */}
          ...
          <div className="flex gap-4 max-w-md mx-auto">
            <button type="button" onClick={handleBackNavigation} className="flex-1 h-14 border border-gray-200 text-gray-600 rounded-xl font-semibold hover:bg-gray-50 transition-colors text-base cursor-pointer">
              Back
            </button>
            <button type="button" onClick={() => setStep("upload")} className="flex-[2] h-14 bg-[#064e3b] text-white rounded-xl font-semibold hover:bg-[#043427] transition-all shadow-sm text-base cursor-pointer">
              Got It — Open Scanner
            </button>
          </div>
        </div>
    ...
    return (
      <div className="max-w-xl mx-auto mt-6 px-4">
        <div className="bg-white rounded-[32px] p-12 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100 text-center space-y-6">
          ...
          <div>
            <h3 className="font-bold text-xl text-gray-900">Processing Scalp Metrics</h3>
            <p className="text-sm text-gray-500 mt-2 leading-relaxed">Our computer vision system is calculating density variations and mapping root coverage fields via Gemini AI...</p>
          </div>
        </div>
      </div>
    );
  ...
  // SCREEN 3: MEDIA VIEW / CAMERA SELECTION DASHBOARD
  return (
      ...
      <div className="bg-white rounded-[32px] p-8 md:p-10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] border border-gray-100">
        ...
        {useCamera ? (
          ...
          <div>
              ...
              <div className="rounded-2xl border border-gray-200 p-2.5 text-center flex flex-col items-center shadow-sm bg-white h-52 justify-between">
                ...
                {images.front ? (
                  <div className="w-full h-36 rounded-xl overflow-hidden border border-gray-100 relative bg-gray-50 flex items-center justify-center">
                    ...
                    <button type="button" onClick={() => removeImage("front")} className="absolute top-1 right-1 bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full shadow-md hover:bg-red-600 cursor-pointer">✕</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => { setActiveCaptureType("front"); fileInputRef.current?.click(); }} className="w-full h-36 border border-dashed border-gray-300 rounded-xl bg-gray-50 flex flex-col items-center justify-center gap-1 cursor-pointer hover:bg-gray-100/50">
                  ...
                  <div className="rounded-2xl border border-gray-200 p-2.5 text-center flex flex-col items-center shadow-sm bg-white h-52 justify-between">
                    ...
                    {images.side ? (
                      <div className="w-full h-36 rounded-xl overflow-hidden border border-gray-100 relative bg-gray-50 flex items-center justify-center">
                        ...
                        <button type="button" onClick={() => removeImage("side")} className="absolute top-1 right-1 bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full shadow-md hover:bg-red-600 cursor-pointer">✕</button>
                      </div>
                    ) : (
                      <button type="button" onClick={() => { setActiveCaptureType("side"); fileInputRef.current?.click(); }} className="w-full h-36 border border-dashed border-gray-300 rounded-xl bg-gray-50 flex flex-col items-center justify-center gap-1 cursor-pointer hover:bg-gray-100/50">
                        <span className="text-lg opacity-40">📸</span>
                  ...
                  <div className="rounded-2xl border border-gray-200 p-2.5 text-center flex flex-col items-center shadow-sm bg-white h-52 justify-between col-span-2 sm:col-span-1 max-w-xs mx-auto sm:max-w-none w-full">
                    ...
                    {images.back ? (
                      <div className="w-full h-36 rounded-xl overflow-hidden border border-gray-100 relative bg-gray-50 flex items-center justify-center">
                        ...
                        <button type="button" onClick={() => removeImage("back")} className="absolute top-1 right-1 bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full shadow-md hover:bg-red-600 cursor-pointer">✕</button>
                      </div>
                    ) : (
                      <button type="button" onClick={() => { setActiveCaptureType("back"); fileInputRef.current?.click(); }} className="w-full h-36 border border-dashed border-gray-300 rounded-xl bg-gray-50 flex flex-col items-center justify-center gap-1 cursor-pointer hover:bg-gray-100/50">
                        <span className="text-lg opacity-40">📸</span>
                ...
                <div className="rounded-2xl border border-gray-200 p-2.5 text-center flex flex-col items-center shadow-sm bg-white h-52 justify-between">
                  ...
                  {images.top ? (
                    <div className="w-full h-36 rounded-xl overflow-hidden border border-gray-100 relative bg-gray-50 flex items-center justify-center">
                      ...
                      <button type="button" onClick={() => removeImage("top")} className="absolute top-1 right-1 bg-red-500 text-white text-[9px] px-2 py-0.5 rounded-full shadow-md hover:bg-red-600 cursor-pointer">✕</button>
                    </div>
                  ) : (
                    <button type="button" onClick={() => { setActiveCaptureType("top"); fileInputRef.current?.click(); }} className="w-full h-36 border border-dashed border-gray-300 rounded-xl bg-gray-50 flex flex-col items-center justify-center gap-1 cursor-pointer hover:bg-gray-100/50">
                      <span className="text-lg opacity-40">📸</span>
            ...
            {((isFemale && (!images.front || !images.side || !images.back)) || (!isFemale && (!images.front || !images.top))) && (
              <div className="flex gap-4 justify-center mb-6">
                <button type="button" onClick={() => { setActiveCaptureType(""); setUseCamera(true); }} className="h-12 px-5 bg-gray-800 text-white font-semibold rounded-full hover:bg-gray-900 transition-all text-sm shadow-sm cursor-pointer">
        ...
        <div className="pt-4 border-t border-gray-100 flex items-center gap-4 w-full">
          <button type="button" onClick={handleBackNavigation} className="flex-1 h-14 border border-gray-200 text-gray-600 rounded-full font-semibold hover:bg-gray-50 transition-colors text-base cursor-pointer">
            Back
          </button>
          <button 
            type="button" 
